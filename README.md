...
npm run test        # the structural battery (1,348) must pass — every suite green,
...
| **Setup & onboarding** | `0.12.0` | ✅ shipped — the §14.28 setup doors (D-049): the `scripts/setup/bootstrap-origin.mjs` migrator-path bootstrap (ONE transaction: origin + credential `must_change` + the first tenant through the founder door; a re-run refuses BY DESIGN), migration 0010 (`user_credential.must_change` + the repository's FIRST SECURITY DEFINER functions — the founder door `setup_create_tenant_with_founder` fail-closed on `is_origin`, and the D-050 pre-tenant membership door `auth_user_tenants` that discharged the latent M11 defect where sign-in's own membership read ran GUC-less under RLS and saw zero rows), the makeSetupAdapter executor (the founder door call, the GUC-fenced account/role/limits commands riding the EXISTING controls_origin_only policies, the per-tenant overview), the /api/setup/* routes Origin-gated at the boundary AND re-proved by the database, the re-authenticated password rotation (must_change clears, every OTHER session tombstones, the Class-N emission per membership), the /setup wizard (the overview's remainingSteps derived by the pure module, the forms, the verbatim refusals), and the first-ingestion upload door running the worker's OWN pipeline in-process (source dropzone, the declared AV posture); @sentinel/module-setup 0.1.0, @sentinel/db 0.12.0, @sentinel/web 0.4.0, SCHEMA_VERSION 0010 |
